...
class MST(nn.Module):
    def __init__(self, in_dim=176, out_dim=176, dim=176, stage=2, num_blocks=[2,4,4]):
...
class MST_Plus_Plus(nn.Module):
    def __init__(self, in_channels=3, out_channels=176, n_feat=176, stage=3):
        super(MST_Plus_Plus, self).__init__()
        self.stage = stage
        self.conv_in = nn.Conv2d(in_channels, n_feat, kernel_size=3, padding=(3 - 1) // 2,bias=False)
        modules_body = [MST(dim=176, stage=2, num_blocks=[1,1,1]) for _ in range(stage)]
...
def train(model, train_loader, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch+1) % 5 == 0:
            sample(model, name, sample_times=8, save_full=True, save_RGB=True, save_dic=f"./experiments/models/MSTPlus/KSC_Corrected/{epoch+1}")
# 生成图像
...
if __name__ == '__main__':
    ...
    paser.add_argument('--image_dir', type=str, default='./experiments/results/MSTPlus/KSC_Corrected', help='directory to save results')

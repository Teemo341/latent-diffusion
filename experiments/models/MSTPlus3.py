...
class MST(nn.Module):
    def __init__(self, in_dim=103, out_dim=103, dim=103, stage=2, num_blocks=[2,4,4]):
...
class MST_Plus_Plus(nn.Module):
    def __init__(self, in_channels=3, out_channels=103, n_feat=103, stage=3):
        ...
        modules_body = [MST(dim=103, stage=2, num_blocks=[1,1,1]) for _ in range(stage)]
...
def train(model, train_loader, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch+1) % 5 == 0:
            sample(model, name, sample_times=8, save_full=True, save_RGB=True, save_dic=f"./experiments/models/MSTPlus/PaviaU/{epoch+1}")
# 生成图像
...
if __name__ == '__main__':
    ...
    paser.add_argument('--image_dir', type=str, default='./experiments/results/MSTPlus/PaviaU', help='directory to save results')

...
class Classifier(nn.Module):
    def __init__(self, latnet_dim, embedding_dim=256, hidden_dim=64, layers=1, classifier_dim=17):
        ...
        self.Conv2 = nn.Sequential(*[nn.Sequential(nn.Conv2d(embedding_dim, embedding_dim, 1),
                                   nn.BatchNorm2d(embedding_dim), nn.LeakyReLU()) for _ in range(layers)])
...
def train(classifier, train_loader, valid_loader=None, num_epochs=100, lr=2e-4, erly_stop=5):
    ...
    scheduler_step = optim.lr_scheduler.CosineAnnealingLR(
    ...
    for epoch in range(num_epochs):
        ...
        for i, input_dic in enumerate(train_loader):
            ...
            acc = (prediction == labels.argmax(dim=1)).sum().item() / \
            ...
            if (i+1) % int(len(train_loader)/1) == 0:  # 一轮验证1次,或者打印1次训练
                if valid_loader is not None:
                    ...
                    for j, input_dic_ in enumerate(valid_loader):
                        ...
                        acc = (prediction_ == labels_.argmax(dim=1)).sum().item(
                        ) / (images_.shape[0] * images_.shape[2] * images_.shape[3])
...
if __name__ == '__main__':
    ...
    paser.add_argument('--image_size', type=int,
                       default=32, help='size of the image')
    # model hyperparam
    paser.add_argument('--embedding_dim', type=int, default=256,
                       help='dimensionality of the embedding space')
    paser.add_argument('--hidden_dim', type=int, default=64,
                       help='dimensionality of the hidden space')
    paser.add_argument('--layers', type=int, default=1,
                       help='number of layers')
    # training hyperparam
    paser.add_argument('--epochs', type=int, default=10,
                       help='number of epochs of training')
    paser.add_argument('--lr', type=float, default=2e-4, help='learning rate')
    paser.add_argument('--save_checkpoint', type=bool,
                       default=True, help='save checkpoint or not')
    paser.add_argument('--load_checkpoint', type=bool,
                       default=False, help='load checkpoint or not')
    paser.add_argument('--checkpoint_dir', type=str,
                       default='./experiments/metric/checkpoints', help='directory to save checkpoints')

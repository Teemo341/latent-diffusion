...
class ImageLogger(Callback):#TODO add HSI image logger
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (self.check_frequency(check_idx) and  # batch_idx % self.batch_freq == 0
            ...
            for k in images:
                if len(images[k].shape)==4: # avoid changing grids
...
class ImageLogger_HSI(ImageLogger):
    def __init__(self, batch_frequency, max_images, visual_channels = [0,1,2], clamp=True, increase_log_steps=True,
                 rescale=True, disabled=False, log_on_batch_idx=False, log_first_step=False,
                 log_images_kwargs=None):
    ...
    #     root = os.path.join(save_dir, "images", split)
    #     for k in images:
    #         print(k, images[k].shape)
    #         if k == 'mask':
    #             grid = torchvision.utils.make_grid(images[k], nrow=4)
    #         elif len(images[k].shape)==3: #already grid
    #             grid = torchvision.utils.make_grid(images[k][self.visual_channels,:,:], nrow=4)
    #         else: 
    #             grid = torchvision.utils.make_grid(images[k][:,self.visual_channels,:,:], nrow=4)
    #             grid_HSI = torchvision.utils.make_grid(images[k], nrow=4)
    #         if self.rescale:
    #             grid = (grid + 1.0) / 2.0  # -1,1 -> 0,1; c,h,w
    #         grid = grid.transpose(0, 1).transpose(1, 2).squeeze(-1)
    #         grid = grid.numpy()
